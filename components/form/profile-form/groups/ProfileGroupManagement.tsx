...
const ProfileGroupManagement = ({ resource, formData }: Props) => {
  return (
    <>
      ...
      <TextView>
        {i18n.t('Managing organization name')}
      </TextView>
      <InputTextField
        value={formData?.profile_organization?.organization_name_that_manages_venue}
        placeholder={i18n.t('Enter the managing organization')}
      ...
      <TextView>
        {i18n.t('Manager phone number')}
      </TextView>
      <InputTextField
        value={formData?.profile_organization?.manager_phone || ''}
        placeholder={i18n.t('Enter the manager phone number')}
        keyboardType="number-pad"
        onChangeText={(value: string) => FormManager.updateField(resource, 'profile_organization', value, ['string'])}
      />
      {FormManager.renderError('profile_organization.manager_phone')}

      <TextView>
        {i18n.t('Manager email address')}
      </TextView>
      <InputTextField
        value={formData?.profile_organization?.manager_email || ''}
        placeholder={i18n.t('Enter the manager email address')}
        onChangeText={(value: string) => FormManager.updateField(resource, 'profile_organization', value, ['string'])}
      />
