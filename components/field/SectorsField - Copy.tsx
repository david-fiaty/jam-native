  ...
  const buildOptions = (optionsData: any): any[] => {    
    return [...(optionsData || [])].map((item: any) => {
      return {
        value: item?.id,
        label: item?.name,
        children: item?.sub_sectors ?? [],
      }
    });
  };

  const onChangeList = async (option: any) => {
    let sublistOptions = buildOptions(listData.find((item: any) => item?.value == option?.value)?.children);
    setSublistData(sublistOptions);

    console.log(sublistOptions);
